...
TEST(test_operator, arrow_test) {
    auto t_ptr = new t;
    t_ptr ->x = 227;
    shared_ptr <t> ptr(t_ptr);
    EXPECT_EQ(ptr->x, t_ptr->x);
}
...
TEST(test_is_move, assign_construct_test) {
    ...
    EXPECT_EQ(std::is_move_constructible<shared_ptr<double>>::value,true);
    EXPECT_EQ(std::is_move_assignable<shared_ptr<t>>::value,true);
    EXPECT_EQ(std::is_move_constructible<shared_ptr<t>>::value,true);
}
